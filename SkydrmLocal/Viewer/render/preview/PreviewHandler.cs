...
namespace Viewer.render.preview
{
    public class PreviewHandler
    {
        ...
        private FileStream mCurrentPreviewHandlerStream;
        private log4net.ILog mLog;
        private static readonly PreviewHandler mInstance = new PreviewHandler();
        ...
        public bool Initialize(string filename, log4net.ILog log)
        {
            bool result = false;
            try
            {
                ...
                InitializePreviewHandlerGUID(filename, out mCurrentPreviewHandlerGUID);
        ...
        private void InitializePreviewHandlerGUID(string filename , out Guid currentPreviewHandlerGUID)
        {
            currentPreviewHandlerGUID = GetPreviewHandlerGUID(filename);
        }
        ...
        private void InitializePreviewHandler(Guid clsid ,out object currentPreviewHandler)
        {
        ...
        public void SetRect(ref Rectangle r)
        {
        ...
        public void DoPreview(string filePath, IntPtr windowHandle, Rectangle rectangle)
        {
            try
            {
                ...
                if (mCurrentPreviewHandler is IInitializeWithFile)
                {
                    ...
                    // passing it to the preview handler if the file is an .xlsx file.
                    if (filePath.EndsWith(".xlsx", StringComparison.CurrentCultureIgnoreCase))
                    {
                        filePath = filePath.Remove(filePath.Length - 5) + ".xlsx";
                    }
                    // passing it to the preview handler if the file is an .xltx file.
                    else if (filePath.EndsWith(".xltx", StringComparison.CurrentCultureIgnoreCase))
                    {
                        filePath = filePath.Remove(filePath.Length - 5) + ".xltx";
                    }
                    // passing it to the preview handler if the file is an .xlsb file.
                    else if (filePath.EndsWith(".xlsb", StringComparison.CurrentCultureIgnoreCase))
                    {
                        filePath = filePath.Remove(filePath.Length - 5) + ".xlsb";
                    }

                    try
                    {
                        mLog.Info("\t\t mCurrentPreviewHandler is IInitializeWithFile \r\n");
                        ((IInitializeWithFile)mCurrentPreviewHandler).Initialize(filePath, 0);
                    }
                    catch (System.Runtime.InteropServices.COMException comex)
                    {
                        mLog.Error(comex.Message, comex);
                        throw comex;
                    }
                }
                else if (mCurrentPreviewHandler is IInitializeWithStream)
                {
                    mLog.Info("\t\t mCurrentPreviewHandler is IInitializeWithStream \r\n");
                    // other handlers want an IStream (in this case, a file stream)
                    mCurrentPreviewHandlerStream = File.Open(filePath, FileMode.Open);
                ...
                {
                    mLog.Info("\t\t mCurrentPreviewHandler is IInitializeWithItem \r\n");
                ...
                if (mCurrentPreviewHandler is IPreviewHandler)
                {
                    mLog.Info("\t\t mCurrentPreviewHandler is IPreviewHandler \r\n");
                    ...
                    mLog.Info("\t\t PreviewHandler SetWindow \r\n");
                    handler.SetWindow(windowHandle, ref rectangle);

                    mLog.Info("\t\t PreviewHandler SetRect \r\n");
                    handler.SetRect(ref rectangle);

                    mLog.Info("\t\t PreviewHandler Do Preview \r\n");
                    handler.DoPreview();
                }
            }
            catch (Exception ex)
