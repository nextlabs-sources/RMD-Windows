...
using Viewer.upgrade.file.component.share.outLook;
    ...
    public class SharePageViewModel : INotifyPropertyChanged
    {
        ...
        public SharePageViewModel(string nxlFilePath, ShareWindow parentWindow)
        {
            ...
            try
            {
                ...
                mNxlFileFingerPrint = mViewerInstance.SdkSession.User.GetNxlFileFingerPrint(nxlFilePath);
        ...
        public SharePageViewModel(NxlFileFingerPrint nxlFileFingerPrint, ShareWindow parentWindow)
        {
            try
            {
                ...
                mViewerInstance = (ViewerApp)ViewerApp.Current;
                mLog = mViewerInstance.Log;
                mNxlFileFingerPrint = nxlFileFingerPrint;
        ...
        private void InitData(NxlFileFingerPrint nxlFileFingerPrint)
        {
         ...
         //   Status = Status | ShareStatus.SHARE_BUTTON_ENABLED;
            ...
            if (CheckExpired(mNxlFileFingerPrint))
            {
                ...
                Message = mViewerInstance.FindResource("Expired").ToString();
            }
        ...
        {
            rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_view.png", mViewerInstance.FindResource("SelectRights_View").ToString()));
            if (rights != null && rights.Count != 0)
            {
                ...
                {
                    rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_edit.png", mViewerInstance.FindResource("SelectRights_Edit").ToString()));
                }
                if (rights.Contains("Print"))
                {
                    rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_print.png", mViewerInstance.FindResource("SelectRights_Print").ToString()));
                }
                if (rights.Contains("Share"))
                {
                    rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_share.png", mViewerInstance.FindResource("SelectRights_Share").ToString()));
                }
                if (rights.Contains("SaveAs"))
                {
                    rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_save_as.png", mViewerInstance.FindResource("SelectRights_SaveAs").ToString()));
                }
                // Fix bug 54210
                if (rights.Contains("Decrypt"))
                {
                    rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_extract.png", mViewerInstance.FindResource("SelectRights_Extract").ToString()));
                }
                if (rights.Contains("Watermark"))
                {
                    rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_watermark.png", mViewerInstance.FindResource("SelectRights_Watermark").ToString()));
                }
            }
            if (isAddValidity)
            {
                rightsItems.Add(new RightsItem(@"/resources/icons/icon_rights_validity.png", mViewerInstance.FindResource("SelectRights_Validity").ToString()));
            }
        }
        ...
        public void Button_Ok(object sender, RoutedEventArgs e)
        {
            ...
            if (!NetworkStatus.IsAvailable)
            {
                ...
                Message =mViewerInstance.FindResource("ShareFileWin_Notify_File_Share_Failed_No_network").ToString();
                return;
            }
        ...
        private void SendLog(bool isAllow)
        {
            try
            {
                ...
                mViewerInstance.SdkSession.User.AddLog(mNxlFileFingerPrint.localPath, NxlOpLog.Share, isAllow);
            }
            catch (Exception ex)
            {
                mLog.Error(ex);
        ...
        public void ReShareCompletedEventHandler(object sender, RunWorkerCompletedEventArgs args)
        {
            ...
            else
            {
                ...
                Message = mViewerInstance.FindResource("Successfully_Shared").ToString();
            }
        }
        ...
        private void ErrorHandler(Exception ex)
        {
            ...
            if (null != rmRestApiException)
            {
                ...
                {
                    Message = mViewerInstance.FindResource("ShareFileWin_Notify_File_Share_Failed_Because_revoked").ToString();
                    MessageNotify.NotifyMsg(mViewerInstance.SdkSession, mNxlFileFingerPrint.name, mViewerInstance.FindResource("ShareFileWin_Notify_File_Share_Failed_Because_revoked").ToString(), EnumMsgNotifyType.PopupBubble, MsgNotifyOperation.SHARE, EnumMsgNotifyResult.Failed, EnumMsgNotifyIcon.Online);
                }
                else
                {
                  //  CommonUtils.ShowBalloonTip(ex.Message, false, mNxlFileFingerPrint.name);
                    Message = ex.Message;
                    MessageNotify.NotifyMsg(mViewerInstance.SdkSession, mNxlFileFingerPrint.name, mViewerInstance.FindResource("Unsuccessful_Shared").ToString(), EnumMsgNotifyType.PopupBubble, MsgNotifyOperation.SHARE, EnumMsgNotifyResult.Failed, EnumMsgNotifyIcon.Online);
                }
            }
            else
            {
        ...
        */
        private void WrapEmail(string email)
        {
            ...
            if (isemailExist)
            {
                MessageNotify.NotifyMsg(mViewerInstance.SdkSession, string.Empty, trimedText + "\n" +mViewerInstance.FindResource("Share_Email_Details").ToString(), EnumMsgNotifyType.PopupBubble, string.Empty, EnumMsgNotifyResult.Succeed, EnumMsgNotifyIcon.Unknown);
               // ToolKit.ShowBalloonTip(trimedText + "\n" + CultureStringInfo.Share_Email_Details, true);
                return;
            }
