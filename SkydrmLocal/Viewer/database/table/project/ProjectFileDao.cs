...
namespace Viewer.database
{
    ...
    public class ProjectFileDao
    {
        ...
        public static KeyValuePair<String, SQLiteParameter[]> Query_SQL_By_Duid(
          int project_table_pk, string rms_duid)
        {
            string sql = @"
              SELECT   
                *
              FROM
                  ProjectFile
              WHERE
                  project_table_pk=@project_table_pk AND rms_duid=@rms_duid; 
