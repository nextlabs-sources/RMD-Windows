...
namespace PdfRender.muPdf
{
    public static class MuPdfWrapper
    {
        ...
        /// <returns></returns>
        public static System.Windows.Size[] GetPageBounds(IPdfSource source, ImageRotation rotation = ImageRotation.None)
        {
        ...
        private sealed class PdfFileStream : IDisposable
        {
            ...
            public PdfFileStream(IPdfSource source)
            {
                ...
                else if(source is MemorySouarce)
                {
                    var ms = (MemorySouarce)source;
                    Context = NativeMethods.NewContext(IntPtr.Zero, IntPtr.Zero, FZ_STORE_DEFAULT); // Creates the context
...
#pragma warning restore 0649
    ...
    public class MemorySouarce: IPdfSource
    {
        public byte[] Bytes { get; private set; }

        public MemorySouarce(byte[] bytes)
        {
            this.Bytes = bytes;
        }
    }
