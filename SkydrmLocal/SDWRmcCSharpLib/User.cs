using SDWRmcCSharpLib.core;
using SDWRmcCSharpLib.core.exception;
using SDWRmcCSharpLib.helper;
using System;
...
namespace SDWRmcCSharpLib
{
    public class User
    {
        ...
        public string ProtectFileToSystemProject(int projectId, string plain, string dest, List<FileRights> rights,
            WaterMarkInfo waterMark, Expiration expiration, UserSelectTags tags)
        {
        ...
        public IntPtr OpenNxlFile(string NxlFilePath)
        {
        ...
        public bool IsValidNxl(IntPtr nxlFilePtr)
        {
            uint rt = Boundary.SDWL_NXL_File_IsValidNxl(nxlFilePtr, out var isNxlFile);
            if (rt != 0)
            {
                ExceptionFactory.BuildThenThrow("SDWL_User_OpenFile", rt);
            }
        ...
        public void CloseNxlFile(IntPtr File)
        {
            uint rt = Boundary.SDWL_User_CloseNxlFile(hUser, File);
            if (rt != 0)
            {
