...
namespace SkydrmLocal.rmc.fileSystem.external
{
    ...
    {
        public INxlFile GetFileFromRepo(string localPath)
        {
            INxlFile ret = null;
            if (string.IsNullOrEmpty(localPath))
            {
                throw new Exception("local path can't be empty.");
            }

            List<IFileRepo> fileRepos = SkydrmLocalApp.Singleton.MainWin.viewModel.FileRepos;
            foreach(var repo in fileRepos)
            {
                if (repo is ProjectRepo)
                {
                    IList<ProjectData> pd = (repo as ProjectRepo).GetAllData();

                    foreach (var one in pd)
                    {
                        foreach (var f in one.FileNodes)
                ...
                else // myVault or sharedWith
                {
                    foreach (var one in repo.GetLocalFiles())
                    {
