...
using SkydrmDesktop.rmc.app.process.office;
using SkydrmDesktop.rmc.app.process.pdf;
...
namespace SkydrmLocal.rmc.Edit
{
    ...
    public class FileEditorHelper
    {
        ...
        public static bool IsHasEditRights(string filePath)
        {
            ...
            try
            {
                var fp = SkydrmApp.Singleton.Rmsdk.User.GetNxlFileFingerPrint(filePath);
        ...
        public static void DoEdit(string nxlFileDiskPath, Action<EditCallBack> OnEditCompleteCallback)
        {
            ...
            var t = new Thread(() => {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        //SkydrmApp.Singleton.ShowBalloonTip(CultureStringInfo.ApplicationFindResource("Common_System_Internal_Error"));
                        SkydrmApp.Singleton.Log.Error("\t\t Some Error Happended In Edit \r\n");
    ...
    public class FileEditor : IEditFile
    {
        ...
        public void DoEdit(string NxlFileLocalPath, Action<EditCallBack> OnEditCompleteCallback)
        {
            ...
            if (!FileEditorHelper.EditingFileMap.ContainsKey(NxlFileLocalPath))
            {
                ...
                //cache token
                SkydrmApp.Rmsdk.User.CacheRPMFileToken(NxlFileLocalPath);
                ...
                SkydrmApp.Rmsdk.User.ForceCloseFile_NoThrow(NxlFileLocalPath);

                Log.Info("\t\t RPM_EditFile \r\n" +
                         "\t\t\t\t NxlFileLocalPath : " + NxlFileLocalPath + "\r\n");
                //decrypting
                string RpmFilePath = SkydrmApp.Rmsdk.RPM_EditFile(NxlFileLocalPath);
                    ...
                    if (null != EditProcess)
                    {
                        ...
                        if (pid != Int32.MaxValue)
                        {
                            ...
                            if (null != Process)
                            {
                                ...
                                SkydrmApp.Rmsdk.RPM_RegisterApp(Process.MainModule.FileName);
                ...
                (bool done) =>
                {
                    if (done)
                    {
                        if (IsFileUnModified())
                        {
                            ...
                            SkydrmApp.Current.Dispatcher.BeginInvoke(OnEditCompleteCallback, false);
                        }
                        else
                        {
                            // Edit finished.
                            // Send edit finish callback with modify params which run on UI thread.
                            SkydrmApp.Current.Dispatcher.BeginInvoke(OnEditCompleteCallback, true);
                        }
                    }
                });
        }
    ...
    public class EditLog
    {
        ...
        private static void SendLog(string LocalDiskPath, NxlOpLog op, bool isAllow)
        {
            ...
            {
                //AddNxlFileLog
                SkydrmApp.Singleton.User.AddNxlFileLog(LocalDiskPath, op, isAllow);
            }
            catch (Exception e)
            {
