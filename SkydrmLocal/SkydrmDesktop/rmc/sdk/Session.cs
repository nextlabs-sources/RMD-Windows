...
namespace SkydrmLocal.rmc.sdk
{
    ...
    public class Session
    {
        ...
        public NxlFileInfo RPMGetFileInfo(string filePath)
        {
            string duid;
            ...
            var rt = Boundary.SDWL_RPM_GetFileInfo(hSession, filePath, out duid, out pArray, out pSize,
                out pArrRights, out pArrRightSize, out watermark, out expiration, out tags, out tokengroup,
                out creatorid, out infoext, out attributes, out isRPMFolder, out isNxlFile);
            if(rt != 0)
            {
                ExceptionFactory.BuildThenThrow("SDWL_RPM_GetFileInfo", rt);
            }

            NxlFileInfo info = new NxlFileInfo();
            info.duid = duid;

            // userRightsAndWatermark
            Dictionary<FileRights, List<WaterMarkInfo>> rightsAndWatermarks = new Dictionary<FileRights, List<WaterMarkInfo>>();
            ...
            info.rightsAndWatermarks = rightsAndWatermarks;

            // rights
            List<FileRights> rightsList = new List<FileRights>();
            if(pArrRightSize > 0)
            {
                IntPtr cur = pArrRights;
                int structSize = Marshal.SizeOf(typeof(int));
                for(int i = 0; i < pArrRightSize; i++)
                {
                    FileRights r = (FileRights)Marshal.ReadInt32(cur);
                    rightsList.Add(r);
                    cur += structSize;
                }
            }
            // Release
            Marshal.FreeCoTaskMem(pArrRights);
            info.rights = rightsList;

            info.watermark = watermark;
            info.expiration = expiration;
            info.tags = tags;
            info.tokengroup = tokengroup;
            info.creatorid = creatorid;
            info.infoext = infoext;
            info.attributes = attributes;
            info.isRPMFolder = isRPMFolder == 1;
            info.isNxlFile = isNxlFile == 1;

            return info;
        }

        public Dictionary<string, List<string>> RPMReadFileTags(string filePath)
        {
            string tags;
        ...
        public void RPM_AddDir(string dir,
                                SDRmRPMFolderOption operation = (SDRmRPMFolderOption.RPMFOLDER_NORMAL | SDRmRPMFolderOption.RPMFOLDER_API),
                                string filetags="")
        {
           // var rt = Boundary.SDWL_RPM_AddRPMDir(Handle, dir, (int)operation);
            var rt = Boundary.SDWL_RPM_AddRPMDir(Handle, dir, (int)operation, filetags);
            if (rt != 0)
            {
        ...
        public void RPM_RemoveDir(string dir, out string errorMsg)
        {
            ...
            if (rt != 0)
            {
                ExceptionFactory.BuildThenThrow("RemoveRPMDir", rt,
                    RmSdkExceptionDomain.Sdk_Common, RmSdkRestMethodKind.Genernal, errorMsg);
            }
        }
        ...
        public bool RPM_AddTrustedApp(string appPath)
        {
            return 0 == Boundary.SDWL_RPM_AddTrustedApp(hSession, appPath);
        }
