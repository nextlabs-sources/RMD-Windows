    ...
    class DataConvert
    {
        ...
        static public NxlFileFingerPrint Convert(User.InternalFingerPrint fp)
        {
            NxlFileFingerPrint v = new NxlFileFingerPrint()
            {
                ...
                tags = DataConvert.ParseClassificationTag(fp.tags),
                rawtags = fp.tags,
                expiration = fp.expiration,
                adhocWatermark = fp.adhocWatermark,
                rights = DataConvert.ParseRights(fp.rights).ToArray(),
        ...
        public static MyVaultMetaData Convert(User.InternalMyVaultMetaData md)
        {
            MyVaultMetaData metaData = new MyVaultMetaData
            {
                ...
                pathId = md.pathId,
                tags = DataConvert.ParseClassificationTag(md.tags),
                rawTags = md.tags,
        ...
        public static WorkspaceMetaData ConvertMetaData(User.InternalWorkSpaceMetaData md)
        {
        ...
        public static List<int> ParseProjectRecipients(string recipients)
        {
            ...
            if (!string.IsNullOrEmpty(recipients))
            {
                ...
                {
                    foreach (var one in recipients.Split(';'))
                    {
                        if (!string.IsNullOrEmpty(one))
                        {
                            ret.Add(int.Parse(one));
                        }
                    }
                }
                else
                {
                    ret.Add(int.Parse(recipients));
                }
            }
        ...
        public static List<uint> ParseSharedWithProjects(string str)
        {
