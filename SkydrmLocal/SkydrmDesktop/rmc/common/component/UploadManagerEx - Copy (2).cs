...
namespace SkydrmLocal.rmc.common.component
{
    ...
    public class UploadManagerEx
    {
        ...
        public void UploadSpecifiedFile(INxlFile nxlFile)
        {
            ...
            ExecuteAsyncUpload(nxlFile);

        }

        private void ExecuteAsyncUpload(INxlFile nxlFile)
        {
            Func<INxlFile, UploadResult> func = new Func<INxlFile, UploadResult>(InnerUpload);
            Action<UploadResult> callback = new Action<UploadResult>(UploadCallback);

            AsyncHelper.RunAsync(func, nxlFile, callback);
        }

        private void UploadCallback(UploadResult uploadResult)
        {
            Count--;

            // remove from uploading list.
            RemoveFromUploadingList(uploadResult.UploadingFile);
        ...
        private const int MAX_TASK_NUM = 5;
        private int Count = 0;
        private void StartUpload(INxlFile nxl)
        {
            ...
            // only the threadPool thread becomes available, can execute the task, so we should do copy(EnCopyQueue).
        ...
        //    {
        //        INxlFile nxl = DeCopyQueue();

        //        Task task = Task.Run(() => { Upload(nxl); });
                
        //        tasks.Add(task);  // 1, 2, 3, 4, 5 --> we create 5 tasks in total.
        ...
        private UploadResult InnerUpload(INxlFile uploadingFile)
        {
            // tmp var
            ...
            return new UploadResult(uploadingFile, originalStatus, bUploadSucceed);
        }
        ...
        private void Upload(object task)
        {
            ...
            ExecuteAsyncUpload(uploadingFile);
        }
        ...
        private int UploadingListCount()
        {
            lock(this)
            {
                return uploadingList.Count;
            }
        }
