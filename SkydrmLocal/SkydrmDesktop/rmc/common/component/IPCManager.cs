    ...
    public class IPCManager
    {
        ...
        public IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            try
            {
                ...
                else if (msg == app.WM_CHECK_IF_ALLOW_LOGOUT)
                {
                    if (!MenuDisableMgr.GetSingleton().IsAllowLogout())
                    {
                        app.ShowBalloonTip(CultureStringInfo.ApplicationFindResource("Notify_PopBubble_Forbid_Logout"), false);
