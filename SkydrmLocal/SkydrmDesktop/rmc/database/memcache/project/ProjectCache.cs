...
namespace SkydrmLocal.rmc.database.memcache.project
{
    class ProjectCache : IProjectCache
    {
        ...
        #region Shared with project file cache field
        private static Dictionary<int, List<SharedWithProjectFile>> mSharedWithProjectFileCaches = new Dictionary<int, List<SharedWithProjectFile>>();
        private static ReaderWriterLockSlim mSharedWithProjectFileLock = new ReaderWriterLockSlim();
        #endregion
        ...
        public void OnInitialize()
        {
            ...
            if (mSharedWithProjectFileCaches == null)
            {
                mSharedWithProjectFileCaches = new Dictionary<int, List<SharedWithProjectFile>>();
                mSharedWithProjectFileLock = new ReaderWriterLockSlim();
            }
        }
        ...
        public void OnDestroy()
        {
            ...
            if(mSharedWithProjectFileCaches != null)
            {
                mSharedWithProjectFileCaches.Clear();
            }
            ...
            try
            {
                ...
                if (data != null && data.Count != 0)
                {
                    var cachedRt = from f in data
                                   where Utils.IsDirectChild(f.Rms_path_id, path) == true
                                   select f;
        ...
        private List<ProjectFile> ConvertFromInternal(InstertProjectFileEx[] inserts, Dictionary<int, int> Project_Id2PK)
        {
            ...
            if (inserts != null && inserts.Length != 0)
            {
                foreach (var i in inserts)
                {
                    ProjectFile f = new ProjectFile
                    {
                        ...
                        // Extend
                        IsShared = i.isShared == 1 ? true : false,
                        IsRevoked = i.isRevoked == 1 ? true : false,
                        SharedWithProjects = i.sharedWithProject,
                        //default value.
                        Is_offline = false,
                        Local_path = "",
                        Operation_status = 4
                ...
                if (data != null && data.Count != 0)
                {
                    foreach (var f in data)
                    {
                        if (f.Id == project_file_table_pk)
                        {
                            ...
                            f.Rms_fileSize = size;
                            f.Rms_lastModifiedTime = lastModifed.ToUniversalTime();
                            return true;
                        }
                    }
                }
            ...
            try
            {
                ...
                if (data != null && data.Count != 0)
                {
                    foreach (var f in data)
                    {
                        if (f.Id == project_file_table_pk)
                        {
                            f.IsShared = newValue == 1 ? true : false;
                            return true;
                        }
                    }
                }
            ...
            try
            {
                ...
                if (data != null && data.Count != 0)
                {
                    foreach (var f in data)
                    {
                        if (f.Id == project_file_table_pk)
                        {
                            f.IsRevoked = newValue == 1 ? true : false;
                            return true;
                        }
                    }
                }
        ...
        public List<ProjectLocalFile> ListProjectLocalFiles(int project_table_pk, string FolderId)
        {
            try
            {
                ...
                if (pFileCaches != null && pFileCaches.Count != 0)
                {
                    ...
                    {
                        if (f.Rms_path_id.Equals(FolderId, StringComparison.OrdinalIgnoreCase))
                        {
                            folders.Add(f);
                        }
                    }
                ...
                if (data != null && data.Count != 0)
                {
                    foreach (var f in data)
                    {
                        ...
                        {
                            // means the project local file 'f' is under the project folder 'ff'.
        ...
        public bool DeleteProjectLocalFile(int project_table_pk, int project_local_file_table_pk)
        {
            try
            {
                ...
                if (data != null && data.Count != 0)
                {
                    ...
                    if (index != -1)
                    {
                        SkydrmApp.Singleton.Log.Info("Delete Project Local File Table Cache:"+ data[index].Name);
                        data.RemoveAt(index);
                    }
                    return index != -1;
                }
        ...
        public bool AddLocalFileToProject(int project_table_pk, string FolderId,
            ...
            int size, DateTime lastModified, string reserved1)
        {
            ...
            int size, DateTime lastModified, string reserved1)
        {
            try
            {
                ...
                var newItem = ConstructNewItem(id, project_table_pk, projectfile_table_pk, name, path, size, lastModified, reserved1);
                if (mProjectLocalFileTableCaches.ContainsKey(key))
                {
                    var data = mProjectLocalFileTableCaches[key];
                    if (data != null)
        ...
        public List<SharedWithProjectFile> ListSharedWithProjectFile(int project_table_pk)
        {
            try
            {
                mSharedWithProjectFileLock.EnterReadLock();

                int key = project_table_pk;
                if (!mSharedWithProjectFileCaches.ContainsKey(key))
                {
                    return null;
                }
                return mSharedWithProjectFileCaches[key];
            }
            finally
            {
                mSharedWithProjectFileLock.ExitReadLock();
            }
        }
        public List<SharedWithProjectFile> ListSharedWithProjectOfflineFile(int project_table_pk)
        {
            try
            {
                ...
                var data = mSharedWithProjectFileCaches[key];
                if (data != null && data.Count != 0)
                {
                    var cachedRt = from f in data
                                   where f.IsOffline == true
                                   select f;
                    return cachedRt.ToList();
                }
                return null;
        ...
        public bool UpsertSharedWithProjectFileBatch(InstertSharedWithProjectFile[] files, Dictionary<int, int> Project_Id2PK)
        {
            try
            {
                mSharedWithProjectFileLock.EnterWriteLock();

                var data = InternalConvert4SharedWithProjectFile(files, Project_Id2PK);
                if (data == null || data.Count == 0)
                {
                    return false;
                }
                foreach (var d in data)
                {
                    ...
                    if (mSharedWithProjectFileCaches.ContainsKey(key))
                    {
                        var value = mSharedWithProjectFileCaches[key];
                        value.Add(d);
                    }
                    else
                    {
                        List<SharedWithProjectFile> newvalue = new List<SharedWithProjectFile>
                            {
                                d
                            };
                        mSharedWithProjectFileCaches.Add(key, newvalue);
                    }
            ...
            {
                mSharedWithProjectFileLock.ExitWriteLock();
            }
        }
        ...
        public bool DeleteSharedWithProjectFile(int project_table_pk, string duid)
        {
            try
            {
                ...
                if(data != null && data.Count != 0)
                {
                    return data.RemoveAll(x => x.RmsDuid == duid) != 0;
                }
        ...
        public bool UpdateSharedWithProjectFileStatus(int project_table_pk, int shared_with_project_file_table_pk, int newStatus)
        {
            try
            {
                ...
                if (data != null && data.Count != 0)
                {
                    foreach (var f in data)
                    {
                        if (f.Id == shared_with_project_file_table_pk)
                        {
                            f.OperationStatus = newStatus;
                            return true;
                        }
                    }
                }
        ...
        public bool PaddingSharedWithProjectFile(int project_table_pk, List<SharedWithProjectFile> data)
        {
            try
            {
                mSharedWithProjectFileLock.EnterWriteLock();
                ...
                if (mSharedWithProjectFileCaches.ContainsKey(key))
                {
                    mSharedWithProjectFileCaches[key] = data;
                    return true;
                }
                //Insert new item into cache.
                mSharedWithProjectFileCaches.Add(key, data);
                return true;
            }
            finally
            {
        ...
        private List<SharedWithProjectFile> InternalConvert4SharedWithProjectFile(InstertSharedWithProjectFile[] inserts,
            Dictionary<int, int> Project_Id2PK)
        {
            List<SharedWithProjectFile> cacheRet = new List<SharedWithProjectFile>();
            if (inserts != null && inserts.Length != 0)
            {
                foreach (var i in inserts)
                {
                    SharedWithProjectFile f = new SharedWithProjectFile
                    {
                        ProjectTablePK = Project_Id2PK[i.project_id],
                        RmsDuid = i.file_duid,
                        RmsName = i.file_name,
                        RmsFileType = i.file_type,
                        RmsFileSize = i.file_size,
                        RmsSharedDate = new DateTime(JavaTimeConverter.ToCSLongTicks(i.shared_date)),
                        RmsSharedBy = i.shared_by,
                        RmsTransactionId = i.transaction_id,
                        RmsTransactionCode = i.transaction_code,
                        RmsSharedUrl = i.shared_url,
                        RmsRightsJson = i.rights_json,
                        RmsIsOwner = (i.is_owner == 1 ? true : false),
                        RmsProtectionType = i.protection_type,
                        RmsSharedByProject = i.shared_by_project,
                        // default value
                        IsOffline = false,
                        LocalPath = "",
                        OperationStatus = 4
