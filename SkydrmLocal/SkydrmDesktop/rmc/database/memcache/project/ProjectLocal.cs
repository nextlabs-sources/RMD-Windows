...
namespace SkydrmLocal.rmc.database.memcache.project
{
    class ProjectLocal : IProjectLocal
    {
        ...
        public List<ProjectFile> ListProjectFile(int project_table_pk, string path)
        {
            ...
            SqliteOpenHelper.ExecuteReader(cs, ProjectFileDao.Query_SQL(ppk, path), (reader) =>
            {
                while (reader.Read())
                {
                    ...
                    if (Utils.IsDirectChild(f.Rms_path_id, path))
                    {
                        rt.Add(f);
                    }
                }
        ...
        public bool UpdateProjectFileWhenOverwriteInLeaveCopy(int project_file_table_pk, string duid,
           int newStatus, long size, DateTime lastModifed)
        {
            string cs = CheckNoNull<string>("UpdateProjectFileWhenOverwriteInLeaveCopy", dataBaseConnectionString);
            int pftk = CheckNoNull<int>("UpdateProjectFileWhenOverwriteInLeaveCopy", project_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs,
                ProjectFileDao.Update_FileWhenOverwrite_LeaveCopy_SQL(pftk, duid, newStatus, size, lastModifed));
            return true;
        }
        ...
        public bool UpdateProjectFileIsShared(int project_file_table_pk, int newValue)
        {
            string cs = CheckNoNull<string>("UpdateProjectFileIsShared", dataBaseConnectionString);
            int pftk = CheckNoNull<int>("UpdateProjectFileIsShared", project_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs,
                ProjectFileDao.Update_IsShared_SQL(pftk, newValue));

            return true;
        }
        public bool UpdateProjectFileIsRevoked(int project_file_table_pk, int newValue)
        {
            string cs = CheckNoNull<string>("UpdateProjectFileIsRevoked", dataBaseConnectionString);
            int pftk = CheckNoNull<int>("UpdateProjectFileIsRevoked", project_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs,
                ProjectFileDao.Update_IsRevoked_SQL(pftk, newValue));

            return true;
        }
        public bool UpdateProjectFileSharedWith(int project_file_table_pk, List<uint> newList)
        {
            string cs = CheckNoNull<string>("UpdateProjectFileSharedWith", dataBaseConnectionString);
            int pftk = CheckNoNull<int>("UpdateProjectFileSharedWith", project_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs,
                ProjectFileDao.Update_SharedWithProjects_SQL(pftk, newList));
        ...
        public bool DeleteProjectLocalFile(int project_local_file_table_pk)
        {
            ...
            {
                SkydrmApp.Singleton.Log.Error("Sqlite execute delete ProjectLocalFile failed, Id:"+ project_local_file_table_pk, e);
                return false;
            }
        ...
        public int AddLocalFileToProject(int project_table_pk, string FolderId, string name, string path, int size, DateTime lastModified, string reserved1)
        {
        ...
        public bool UpdateProjectLocalFileName(int project_local_file_table_pk, string newName)
        {
            string cs = CheckNoNull<string>("UpdateProjectLocalFileName", dataBaseConnectionString);
            int plfk = CheckNoNull<int>("UpdateProjectLocalFileName", project_local_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs,
                ProjectLocalFileDao.Update_File_Name_SQL(plfk, newName));

            return true;
        }
        public bool UpdateProjectLocalFileLocalPath(int project_local_file_table_pk, string localPath)
        {
            string cs = CheckNoNull<string>("UpdateProjectLocalFileLocalPath", dataBaseConnectionString);
            int plfk = CheckNoNull<int>("UpdateProjectLocalFileLocalPath", project_local_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs,
                ProjectLocalFileDao.Update_File_LocalPath_SQL(plfk, localPath));

            return true;
        }
        public bool UpdateProjectLocalFileReserved1(int project_local_file_table_pk, string reserved1)
        {
            string cs = CheckNoNull<string>("UpdateProjectLocalFileFileExist", dataBaseConnectionString);
            int plfk = CheckNoNull<int>("UpdateProjectLocalFileFileExist", project_local_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs,
                ProjectLocalFileDao.Update_File_Reserved1_SQL(plfk, reserved1));
        ...
        {
            string cs = CheckNoNull<string>("ListSharedWithProjectFile", dataBaseConnectionString);
            int ppk = CheckNoNull<int>("ListSharedWithProjectFile", project_table_pk);
            List<SharedWithProjectFile> rt = new List<SharedWithProjectFile>();
            SqliteOpenHelper.ExecuteReader(cs, SharedWithProjectDao.Query_SQL(ppk), (reader) =>
            {
                while (reader.Read())
                {
                    rt.Add(SharedWithProjectFile.NewByReader(reader));
                }
            });
            return rt;
        }
        ...
        {
            string cs = CheckNoNull<string>("ListSharedWithProjectOfflineFile", dataBaseConnectionString);
            int ppk = CheckNoNull<int>("ListSharedWithProjectOfflineFile", project_table_pk);
            List<SharedWithProjectFile> rt = new List<SharedWithProjectFile>();
            SqliteOpenHelper.ExecuteReader(cs, SharedWithProjectDao.Query_SQL(ppk), (reader) =>
            {
                while (reader.Read())
        ...
        {
            string cs = CheckNoNull<string>("UpsertSharedWithProjectFileBatch", dataBaseConnectionString);

            List<KeyValuePair<string, SQLiteParameter[]>> sqls = new List<KeyValuePair<string, SQLiteParameter[]>>();
            foreach (var i in files)
            {
                // Java time mills is not same as C# time mills
                sqls.Add(SharedWithProjectDao.Upsert_SQL(Project_Id2PK[i.project_id],
                    i.file_duid, 
                    i.file_name, 
                    i.file_type, 
                    i.file_size,
        ...
        // Delete file
        public bool DeleteSharedWithProjectFile(string duid)
        {
            string cs = CheckNoNull<string>("UpdateSharedWithProjectFileStatus", dataBaseConnectionString);

            SqliteOpenHelper.ExecuteNonQuery(cs, SharedWithProjectDao.Delete_Item_SQL(duid));
            return true;
        }

        // File status
        public bool UpdateSharedWithProjectFileStatus(int shared_with_project_file_table_pk, int newStatus)
        {
            string cs = CheckNoNull<string>("UpdateSharedWithProjectFileStatus", dataBaseConnectionString);
            int swpft_pk = CheckNoNull<int>("UpdateSharedWithProjectFileStatus", shared_with_project_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs, SharedWithProjectDao.Update_FileStatus_SQL(swpft_pk, newStatus));
            return true;
        }
        // Offline mark
        public bool UpdateSharedWithProjectFileOfflineMark(int shared_with_project_file_table_pk, bool newMark)
        {
            string cs = CheckNoNull<string>("UpdateSharedWithProjectFileOfflineMark", dataBaseConnectionString);
            int swpft_pk = CheckNoNull<int>("UpdateSharedWithProjectFileOfflineMark", shared_with_project_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs, SharedWithProjectDao.Update_IsOffline_SQL(swpft_pk, newMark));
            return true;
        }
        // Local path
        public bool UpdateSharedWithProjectFileLocalpath(int shared_with_project_file_table_pk, string newPath)
        {
            string cs = CheckNoNull<string>("UpdateSharedWithProjectFileLocalpath", dataBaseConnectionString);
            int swpft_pk = CheckNoNull<int>("UpdateSharedWithProjectFileLocalpath", shared_with_project_file_table_pk);

            SqliteOpenHelper.ExecuteNonQuery(cs, SharedWithProjectDao.Update_LocalPath_SQL(swpft_pk, newPath));
            return true;
        }
