...
namespace SkydrmLocal.rmc.database2.table.myvault
{
    public class MyVaultFile
    {
        ...
        // Reserved1 use to flag that indicate myvault offline file whether is overwritten from local in leave copy model.
    ...
    public class MyVaultFileDao
    {
        ...
        {
            string modifiedTimeStr = lastmodifed.ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss");
            ...
            SQLiteParameter[] parameters = {
                ...
                new SQLiteParameter("@status",status),
                new SQLiteParameter("@size",size),
                new SQLiteParameter("@last_modified_time", modifiedTimeStr),
                new SQLiteParameter("@shared_time", modifiedTimeStr)
            };

            return new KeyValuePair<String, SQLiteParameter[]>(sql, parameters);
        }

        public static KeyValuePair<String, SQLiteParameter[]> Update_Resereved1_SQL(
         int table_pk, string reserved1)
        {
            string sql = @"UPDATE MyVaultFile
                           SET 
                                reserved1=@reserved1
