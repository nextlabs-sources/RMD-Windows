...
namespace SkydrmDesktop.rmc.database.table.workspace
{
    ...
    public class WorkSpaceFileDao
    {
            ...
            int modifiedBy, string modifiedByName, string modifiedByEmail)
        {
            ...
            // Note: when file is overwrite, file_id still is the same (duid is different), 
            // so we should update some fields according to it.
            string sql = @"     
               UPDATE 
                    WorkSpaceFile
                SET
                    ...
                    rms_size=@size,
                    rms_duid=@duid,
                    rms_path_display=@path_display,
                    rms_path_id=@path_id,
                    rms_name=@name,
                    rms_file_type=@file_type,
                    rms_created=@created_time,
                    rms_is_folder=@is_folder,
                    rms_owner_id=@owner_id,
                    rms_owner_display_name=@owner_display_name,
                    rms_owner_email=@owner_email,
                    rms_modified_by=@modified_by,
                    rms_modified_by_name=@modified_by_name,
                    rms_modified_by_email=@modified_by_email
                WHERE
        ...
        /// <returns></returns>
        public static KeyValuePair<String, SQLiteParameter[]> Query_SQL(int user_tb_pk, string folderPathId)
        {
            ...
            {
                new SQLiteParameter("@user_table_pk", user_tb_pk),
                new SQLiteParameter("@path", folderPathId+'%')
            };

            return new KeyValuePair<string, SQLiteParameter[]>(sql, paras);               
        }

        public static KeyValuePair<String, SQLiteParameter[]> QueryAll_SQL(int user_tb_pk)
        {
            string sql = @"SELECT * FROM
                                WorkSpaceFile 
                           WHERE
        ...
        public static KeyValuePair<String, SQLiteParameter[]> Update_NXL_For_Overwrite_LeaveCopyModel(
           int table_pk, string duid, int status, long size, DateTime lastmodifed)
        {
