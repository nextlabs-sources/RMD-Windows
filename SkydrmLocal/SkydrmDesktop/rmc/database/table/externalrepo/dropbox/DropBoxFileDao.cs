...
{
    class DropBoxFileDao
    {
        public static readonly string SQL_Create_Table_DropBoxFile = @"
            CREATE TABLE IF NOT EXISTS DropBoxFile (
                ...
                int rms_external_repo_table_pk)
        {
            string sql = @"
                INSERT OR IGNORE INTO 
                    DropBoxFile(rms_external_repo_table_pk,ser_file_id,ser_display_path,ser_cloud_pathid)
        ...
        public static KeyValuePair<String, SQLiteParameter[]> Query_SQL(
                int rms_external_repo_table_pk, string cloudDisplayPath)
        {
            string sql = @"
              SELECT   
                *
            ...
            SQLiteParameter[] parameters = {
                new SQLiteParameter("@rms_external_repo_table_pk",rms_external_repo_table_pk),
                new SQLiteParameter("@path",cloudDisplayPath+'%')
            };
            return new KeyValuePair<string, SQLiteParameter[]>(sql, parameters);
        }
            ...
            string ser_cloud_pathid, int is_nxl)
        {
            ...
            string sql = @"
                   ...
                   ---------if no updated happeded, then insert one--------------------------
                    INSERT INTO  
                            DropBoxFile(rms_external_repo_table_pk,
                                            ser_file_id,
                                            ser_isFolder,
                                            ser_file_name,
                                            ser_size,
