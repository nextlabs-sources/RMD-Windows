...
namespace SkydrmDesktop.rmc.database.table.externalrepo.Box
{
    class BoxFileDao
    {
        public static readonly string SQL_Create_Table_BoxFile = @"
            CREATE TABLE IF NOT EXISTS BoxFile (
        ...
        int rms_external_repo_table_pk)
        {
            string sql = @"
                INSERT OR IGNORE INTO 
                    BoxFile(rms_external_repo_table_pk,ser_file_id,ser_display_path,ser_cloud_pathid)
        ...
        public static KeyValuePair<String, SQLiteParameter[]> Query_SQL(
                int rms_external_repo_table_pk, string cloudPathId)
        {
            string sql = @"
            SELECT   
                *
            ...
            WHERE
                 ...
                 ser_cloud_pathid like @path;                 
            ";
            SQLiteParameter[] parameters = {
                new SQLiteParameter("@rms_external_repo_table_pk",rms_external_repo_table_pk),
                new SQLiteParameter("@path",cloudPathId+'%')
            };
            return new KeyValuePair<string, SQLiteParameter[]>(sql, parameters);
        }
            ...
            string ser_cloud_pathid, int is_nxl)
        {
            ...
            string sql = @"
                   ...
                   ---------if no updated happeded, then insert one--------------------------
                    INSERT INTO  
                            BoxFile(rms_external_repo_table_pk,
                                            ser_file_id,
                                            ser_isFolder,
                                            ser_file_name,
                                            ser_size,
        ...
        public static KeyValuePair<String, SQLiteParameter[]> Query_CloudDisplayPath(int rms_external_repo_table_pk, string fileId)
        {
            string sql = @"
              SELECT   
                ser_display_path
            FROM
                BoxFile
            WHERE
                 rms_external_repo_table_pk=@rms_external_repo_table_pk AND ser_file_id = @path;                
            ";
            SQLiteParameter[] parameters = {
                new SQLiteParameter("@rms_external_repo_table_pk",rms_external_repo_table_pk),
                new SQLiteParameter("@path",fileId)
            };
            return new KeyValuePair<string, SQLiteParameter[]>(sql, parameters);
        }


        public static KeyValuePair<String, SQLiteParameter[]> Query_CloudPathId(int rms_external_repo_table_pk, string fileId)
        {
            string sql = @"
              SELECT   
                ser_cloud_pathid
        ...
        public static KeyValuePair<String, SQLiteParameter[]> Query_CloudFileId_SQL(
            int rms_external_repo_table_pk,
            int this_table_pk)
        {
            string sql = @"
