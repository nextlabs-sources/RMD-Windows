...
namespace SkydrmLocal.rmc.drive
{
    ...
    public class RightsManagementService 
    {
        ...
        public static string GenerateDecryptFilePath(string RPMFolder, string NxlFilePath, DecryptIntent decryptIntent)
        {
            ...
            SkydrmApp.Singleton.Log.Info($"Create directory {GuidDirectory}");
            Directory.CreateDirectory(GuidDirectory);
            SkydrmApp.Singleton.Log.Info("Create directory successfully");
        ...
        public static void DecryptNXLFile (SkydrmApp skydrmLocalApp, string nxlfileLocalPath, string decryptedFilePath)
        {
        ...
        public static void AsynDecryptNXLFile(SkydrmApp skydrmLocalApp, string nxlfileLocalPath, string decryptedFilePath)
        {
        ...
        public static void RPMDeleteDirectory(SkydrmApp skydrmLocalApp, string directoryPath)
        {
            SkydrmApp.Singleton.Log.Info("\t\t Enter:  RPMDeleteDirectory \r\n");
            ...
            try
            {
                ...
                SkydrmApp.Singleton.Log.Info($"\t\t Directory: {directoryPath} get all files \r\n");
                string[] allFilePath = Directory.GetFiles(directoryPath);
                SkydrmApp.Singleton.Log.Info($"\t\t Directory get all files successfuly, count {allFilePath.Count()} \r\n");

                foreach (string filePath in allFilePath)
                {
                    SkydrmApp.Singleton.Log.Info("\t\t Begin: RPM_DeleteFile \r\n");
                    Rmsdk.RPM_DeleteFile(filePath);
                    SkydrmApp.Singleton.Log.Info("\t\t End: RPM_DeleteFile \r\n");
                }

                SkydrmApp.Singleton.Log.Info($"\t\t Directory: {directoryPath} get all directories \r\n");
                string[] allSubdirectory = Directory.GetDirectories(directoryPath);
                SkydrmApp.Singleton.Log.Info($"\t\t Directory get all directories successfuly, count {allSubdirectory.Count()} \r\n");
                ...
                SkydrmApp.Singleton.Log.Info("\t\t Leave:  RPMDeleteDirectory \r\n");
            }
            catch (Exception ex)
            {
        ...
        public static void RPMDeleteFile(SkydrmApp skydrmLocalApp, string filePath)
        {
            if (null == skydrmLocalApp)
            {
                return;
