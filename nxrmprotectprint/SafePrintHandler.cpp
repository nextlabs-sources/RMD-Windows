...
namespace {
	...
	inline bool IsNXLFile(const wchar_t* wszFilePath) {
		SafePrintHandler& handler = SafePrintHandler::Instance();
		if (handler.m_pRmcInstance == NULL)
		{
			return false;
		}
		unsigned int dirstatus = 0;
		bool filestatus = false;
		if (handler.m_pRmcInstance->RPMGetFileStatus(wszFilePath, &dirstatus, &filestatus).GetCode() != 0) {
			return false;
		}
		if (filestatus) {
			return is_file_exist(wszFilePath);
		}
		return false;
	}
	...
	inline std::wstring GetProcessName(unsigned long processId)
	{
		DWORD		dwErr = 0;
	...
	{
		if (0 == ::MoveFileExW(lpExistingFileName, lpNewFileName, MOVEFILE_COPY_ALLOWED | MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
		{
		...
		//	std::wstring processname = GetProcessName(0);
		...
		//	SafePrintHandler& handler = SafePrintHandler::Instance();
		//	if (NULL == handler.m_pRmcInstance) {
		//		return false;
		//	}
		//	SDWLResult res = handler.m_pRmcInstance->RPMCopyFile(lpExistingFileName, lpNewFileName, true);
...
pfn_EnumJobNamedProperties my_EnumJobNamedProperties;
...
bool SafePrintHandler::Is_Printer_Name_In_Trusted_List(const std::wstring printer_name)
{
	...
	*/
	...
	{
		::Registry r;
		::Registry::param param(LR"_(SOFTWARE\NextLabs\SkyDRM)_");
	...
	return ::iconstain(trusted_printers, printer_name);
}
...
bool SafePrintHandler::NotifyMessage(const std::wstring &app, const std::wstring &target, const std::wstring &msg,
...
bool SafePrintHandler::Popup_Message(const std::wstring& message, const std::wstring &nxlpath, uint32_t result)
{
	std::experimental::filesystem::path _filename(nxlpath);
	return NotifyMessage(L"nxrmprotectprint", _filename.filename(), message, 1, result);
}
...
bool SafePrintHandler::CovnerFrom_Wrapper(const std::wstring& srcplainfile,
	...
	std::wstring& outGeneratedFile)
{
	...
	//wchar_t folder[1024];
	////wchar_t* folder = new wchar_t[srcplainfile.size() + 1];
	//wcscpy_s(folder, 1024, srcplainfile.c_str());
	////folder[srcplainfile.size()] = '\0';
	//::PathRemoveFileSpecW(folder);
	//std::wstring out(folder);

	if (m_pUser->ProtectFileFrom(srcplainfile, originalnxlfile, out).GetCode() != 0) {
		return false;
	}
...
std::wstring SafePrintHandler::Convert_PrintedFile_To_NXL(const std::wstring& org_plain_file_path, const std::wstring& current_nxl_template)
{
	...
	if (!IsNXLFile(current_nxl_template.c_str())) {
		return new_nxl_file;
	}
	...
	if (!CovnerFrom_Wrapper(org_file_in_tmp_folder, current_nxl_template, new_nxl_file)) {
