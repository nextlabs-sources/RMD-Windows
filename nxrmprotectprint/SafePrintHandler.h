...
public: // Data
	...
	bool NotifyMessage(const std::wstring &app, const std::wstring &target, const std::wstring &msg,
		uint32_t msgtype, uint32_t result);

	bool Popup_Message(const std::wstring& message, const std::wstring &nxlpath, uint32_t result);
...
private:
	...
	bool CovnerFrom_Wrapper(const std::wstring& srcplainfile, const std::wstring& originalnxlfile, std::wstring& outGeneratedFile);
