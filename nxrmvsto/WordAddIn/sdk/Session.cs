...
namespace WordAddIn.sdk
{
    public class Session
    {
        ...
        public void RPMGetFileRights(string filePath, out List<FileRights> rights, out WaterMarkInfo watermark)
        {
            ...
            var rt = Boundary.SDWL_RPM_GetFileRights(hSession, filePath, out addr, out size, out watermark, 0);
            if (rt != 0)
            {
        ...
        public void RPM_copyFile(string srcPath, string destPath, bool IsDeleteSource)
        {
            var rt = Boundary.SDWL_RPM_CopyFile(hSession, srcPath,destPath,IsDeleteSource);
            if (rt != 0)
            {
        ...
        public bool RMP_IsSafeFolder(string path)
        {
            ...
            int option;
            string tags;
            var rt = Boundary.SDWL_RPM_IsSafeFolder(hSession, path, out isSaveFolder, out option, out tags);
            if (rt != 0)
            {
